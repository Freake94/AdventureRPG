...
static void window_create(Window* window, const char* title, int width, int height, int bits) {
    window->width   = width;
    ...
    pixel_format = ChoosePixelFormat(window->hdc, &pdf);

    SetPixelFormat(window->hdc, pixel_format, &pdf);
    ...
    window->mouse.pos[0] = mouse_pos.x;
    window->mouse.pos[1] = mouse_pos.y;

    window->timer = timer_create();
}

static void window_update(Window* window) {
    POINT mouse_pos;
    GetCursorPos(&mouse_pos);
    ScreenToClient(window->hwnd, &mouse_pos);

    window->mouse.delta[0] = mouse_pos.x - window->mouse.pos[0];
    window->mouse.delta[1] = mouse_pos.y - window->mouse.pos[1];

    window->mouse.pos[0] = mouse_pos.x;
    window->mouse.pos[1] = mouse_pos.y;

    if (window->mouse.keep_in_center) {
        /*
        POINT point = {
            window->width  / 2,
            window->height / 2
    ...
    memset(window->keyboard.pressed,  0, sizeof (window->keyboard.pressed));
    memset(window->keyboard.released, 0, sizeof (window->keyboard.released));
    ...
    while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
        ...
        switch (msg.message) {
            ...
            case WM_KEYUP: {
                window->keyboard.state[msg.wParam] = 0;
                window->keyboard.released[msg.wParam] = 1;
                break;
            }
            case WM_KEYDOWN: {
                window->keyboard.state[msg.wParam] = 1;
                window->keyboard.pressed[msg.wParam] = 1;
                break;
            }
            case WM_LBUTTONDOWN: {
    ...
    window->elapsed_secs = timer_restart(&window->timer);
    window->total_time   = timer_get_current();

    ShowCursor(window->mouse.visible);
